...
Materials, available at <https://github.com/felixgolcher/rmd_workshop/tree/materials>, have the following structure
...
XXX overwrite! This and the following slides
...
In our case, students have done that by now.
...
- `rmarkdown` `r cp("rmarkdown")`, for automating the process of converting R Markdown documents into other formats

--

- `tinytex` `r cp("tinytex")`, for PDF outputs
    - requires an additional step to install
    - alternative: a $\TeX$ / $\LaTeX$ system installed on your computer
    - `tinytex` is much smaller, and no fuss to install.
    - $\LaTeX$ packages that are not installed, are added on the fly.
...
* Click the ▼ to the right of the `Knit` button
  ...
  - you see another document appear
  - don't be surprised that it's a `PDF`.
...
<sup>**</sup> RStudio comes with a copy of `pandoc` ([http://pandoc.org](http://pandoc.org)), which is __not__ an R package, so that you do not have to install it separately. 
...
* Here we will focus on research papers only.
...
Ich denke, das sollte ich rausnehmen.
...
df <- read.csv("../image/journals.csv") %>% 
