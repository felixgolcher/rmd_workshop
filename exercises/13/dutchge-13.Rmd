...
```{r}
m0 <- lm(DurationOfPrefix~log(Frequency), data=durationsGe)
m1 <- update(m0, .~. + NumberSegmentsOnset)
```

We computed to linear models with the `lm` function of R and the model formulas 

* `r Reduce(paste, deparse(formula(m0)))` and
* `r Reduce(paste, deparse(formula(m1)))`. 

The table compares both models.

```{r modtab, results='asis', echo=FALSE}
stargazer(data=list(m0, m1),
          type = "latex",
          title = "our models",
          keep.stat = "adj.rsq",
          omit="Constant",
...
* model formulas can be stringified with ``Reduce(paste, deparse(formula(yourmodel)))``.
